...
class TestPsqlMetisUserDao {
  ...
  void createMetisUser() {
    ...
    psqlMetisUserDao.createMetisUser(new MetisUserRecord());
  ...
  void createMetisUserThrowsExceptionOnCommit() {
    ...
    assertThrows(TransactionException.class, () -> psqlMetisUserDao.createMetisUser(new MetisUserRecord()));
  ...
  void updateMetisUser() {
    when(session.beginTransaction()).thenReturn(transaction);
    psqlMetisUserDao.updateMetisUser(new MetisUserRecord());
  ...
  void updateMetisUserThrowsExceptionOnCommit() {
    when(session.beginTransaction()).thenReturn(transaction);
    doThrow(new RuntimeException("Exception")).when(transaction).commit();
    assertThrows(TransactionException.class, () -> psqlMetisUserDao.updateMetisUser(new MetisUserRecord()));
  ...
  void getMetisUserByEmail() {
    ...
    ArrayList<MetisUserRecord> metisUsers = new ArrayList<>(1);
    metisUsers.add(new MetisUserRecord());
  ...
  void getAllMetisUsers() {
    final CriteriaBuilder builder = Mockito.mock(CriteriaBuilder.class);
    final CriteriaQuery<MetisUserRecord> criteriaQuery = Mockito.mock(CriteriaQuery.class);
    final Query<MetisUserRecord> query = Mockito.mock(Query.class);
    ArrayList<MetisUserRecord> metisUsers = new ArrayList<>(1);
    metisUsers.add(new MetisUserRecord());

    when(session.getCriteriaBuilder()).thenReturn(builder);
    when(builder.createQuery(MetisUserRecord.class)).thenReturn(criteriaQuery);
    when(session.createQuery(criteriaQuery)).thenReturn(query);
