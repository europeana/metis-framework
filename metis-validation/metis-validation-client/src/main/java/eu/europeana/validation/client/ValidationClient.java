...
/**
 * Rest client for the Schema validation REST API Created by ymamakis on 8/1/16.
 */
public class ValidationClient {
  ...
  public ValidationResult validateRecord(String schemaName, String record) {
    ...
    return template.postForEntity(
        validationEndpoint + RestEndpoints.resolve(RestEndpoints.SCHEMA_VALIDATE, Collections
            .singletonList(schemaName)), entity, ValidationResult.class).getBody();
  }
  ...
  public ValidationResultList validateRecordsInFile(String schemaName, File file) {
    ...
    return template.postForObject(validationEndpoint + RestEndpoints
            .resolve(RestEndpoints.SCHEMA_BATCH_VALIDATE, Collections.singletonList(schemaName)),
