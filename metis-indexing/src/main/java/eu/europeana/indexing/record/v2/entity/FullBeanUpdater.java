...
public class FullBeanUpdater extends AbstractMongoObjectUpdater<FullBeanImpl, Void> {
   ...
   * @param createdDate The date that would represent the created date if it already existed,
   * e.g. from a redirected record
   * @param mongoServer The mongo server.
   * @return The updated entity.
   */
  public final FullBeanImpl update(FullBeanImpl newEntity, Date recordDate, Date createdDate,
      RecordDao mongoServer) {
    return update(newEntity, null, recordDate, createdDate, mongoServer);
  }
  ...
  protected void update(MongoPropertyUpdater<FullBeanImpl> propertyUpdater,
    ...
    propertyUpdater.updateObjectList("persistentIdentifiers", FullBeanImpl::getPersistentIdentifiers);
  }
