...
public class SolrUtilsTest {
  ...
  public void testAddValues() {
    ...
    SolrUtils.addValue(document, EdmLabel.PL_WGS84_POS_LAT, "A");
    ...
    SolrUtils.addValue(document, EdmLabel.PL_WGS84_POS_LAT, 0.0F);
    assertEquals(1, document.getFieldNames().size());
    ...
    SolrUtils.addValues(document, EdmLabel.PL_WGS84_POS_LONG, new String[] {"C", "D"});
    ...
    SolrUtils.addValues(document, EdmLabel.PL_WGS84_POS_ALT, inputMap);
  ...
  public void testAddNullValues() {
    ...
    // Add null values
    SolrUtils.addValue(document, EdmLabel.PL_WGS84_POS_ALT, (Float) null);
    SolrUtils.addValue(document, EdmLabel.PL_WGS84_POS_ALT, (String) null);
    SolrUtils.addValues(document, EdmLabel.PL_WGS84_POS_ALT, (String[]) null);
    SolrUtils.addValues(document, EdmLabel.PL_WGS84_POS_ALT, (Map<String, List<String>>) null);

    // Add map with null values
    Map<String, List<String>> input = new HashMap<>();
    input.put("A", null);
    SolrUtils.addValues(document, EdmLabel.PL_WGS84_POS_ALT, input);
  ...
  public void testRightsForMap() {
    ...
    // Test null map
    final Stream<String> fromNullMap = SolrUtils.getRightsFromMap(null);
    assertNotNull(fromNullMap);
    assertEquals(0L, fromNullMap.count());

    // Test empty map
    final Stream<String> fromEmptyMap = SolrUtils.getRightsFromMap(Collections.emptyMap());
    ...
    final Stream<String> fromFilledMap = SolrUtils.getRightsFromMap(rights);
  ...
  public void testHasLicenseForRights() {
    ...
    // no rights required
    assertTrue(SolrUtils.hasLicenseForRights(Collections.emptyMap(), license -> false));
    assertTrue(SolrUtils.hasLicenseForRights(Collections.emptyMap(), license -> true));
    ...
    assertFalse(SolrUtils.hasLicenseForRights(rights, contradiction));
    ...
    assertTrue(SolrUtils.hasLicenseForRights(rights, hasLicense));
  }
