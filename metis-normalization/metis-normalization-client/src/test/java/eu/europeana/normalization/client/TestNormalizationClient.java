...
public class TestNormalizationClient {
  ...
  public static void main(String[] args) {
    try {
      ...
      NormalizationClient client = new NormalizationClient("http://localhost:8080/");

      List<String> recs = new ArrayList<>();

      //change to correct file location
      FileInputStream in = new FileInputStream(new File(
          "metis-normalization/metis-normalization-client/src/test/samples/edm-record.xml"));
      recs.add(IOUtils.toString(in, "UTF-8"));
      in.close();

      //change to correct file location
      in = new FileInputStream(new File(
          "metis-normalization/metis-normalization-client/src/test/samples/edm-record-internal.xml"));
      recs.add(IOUtils.toString(in, "UTF-8"));
      in.close();
