...
import static eu.europeana.metis.utils.CompressedSecureFile.MAX_ENTRY_SIZE;
import static eu.europeana.metis.utils.CompressedSecureFile.MIN_INFLATE_RATIO;
...
public class CompressedFileInputStreamThreshold extends FilterInputStream {

  // don't alert for expanded sizes smaller than 50k
  private static final long GRACE_ENTRY_SIZE = 50 * 1024L;
  ...
  public CompressedFileInputStreamThreshold(InputStream is) {
    super(is);
  ...
  /**
   * De-/activate threshold check. A disabled guard might make sense, when POI is processing its own temporary data (see #59743)
   *
  ...
  private void checkThreshold() throws IOException {
    ...
    throw new IOException(
        String.format(Locale.ROOT, MIN_INFLATE_RATIO_MSG, payloadSize, rawSize, ratio, MIN_INFLATE_RATIO, entryName));
  }
    ...
    try {
      ...
      if (ze.getMessage().startsWith("Unexpected record signature")) {
        throw new IllegalArgumentException(
            "No valid entries or contents found, this is not a valid compressed file", ze);
      }
      throw ze;
