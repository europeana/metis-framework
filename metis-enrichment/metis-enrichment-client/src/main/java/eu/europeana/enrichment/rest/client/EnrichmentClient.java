...
public class EnrichmentClient {
  ...
  public EnrichmentResultList enrich(List<InputValue> values) {
    ...
    try {
      return template.postForObject(endpoint + ENRICHMENT_ENRICH, inList, EnrichmentResultList.class);
  ...
  public EnrichmentBase getByUri(String uri) {
    UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(endpoint + ENRICHMENT_BYURI)
        .queryParam("uri", uri);
