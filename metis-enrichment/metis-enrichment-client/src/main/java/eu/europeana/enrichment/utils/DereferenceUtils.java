...
public final class DereferenceUtils {
    ...
    private static void dereferenceAggregation(Aggregation aggregation, final Set<String> result) {
        AggregationFieldType.DATA_PROVIDER.extractFields(aggregation).forEach(item -> convertValue(item, RESOURCE_OR_LITERAL_EXTRACTOR, result));
        AggregationFieldType.PROVIDER.extractFields(aggregation).forEach(item -> convertValue(item, RESOURCE_OR_LITERAL_EXTRACTOR, result));
        AggregationFieldType.INTERMEDIATE_PROVIDER.extractFields(aggregation).forEach(item -> convertValue(item, RESOURCE_OR_LITERAL_EXTRACTOR, result));
    }
